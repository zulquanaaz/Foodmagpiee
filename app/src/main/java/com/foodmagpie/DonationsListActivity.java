...
public class DonationsListActivity extends AppCompatActivity {
    ...
    public void getDonations() {
        loading = new ProgressDialog(DonationsListActivity.this);
        ...
        call.enqueue(new Callback<List<DonationPojo>>() {
            ...
            public void onResponse(Call<List<DonationPojo>> call, Response<List<DonationPojo>> response) {
                ...
                if (response.body() == null) {
                    Toast.makeText(DonationsListActivity.this, "No data found", Toast.LENGTH_SHORT).show();
                } else {
                    donationPojos = response.body();
                    donations_list.setAdapter(new DonationsListAdapter(donationPojos, DonationsListActivity.this));
                }
            }
            ...
            public void onFailure(Call<List<DonationPojo>> call, Throwable t) {
                loading.dismiss();
                Toast.makeText(DonationsListActivity.this, "Something went wrong...Please contact admin !", Toast.LENGTH_SHORT).show();
            }
        });
    }
